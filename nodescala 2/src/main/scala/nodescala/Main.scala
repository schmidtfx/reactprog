...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val userInterrupted: Future[String] = Future.userInput("Please enter").continue {
      case Success(x) => "You entered... %s".format(x)
    ...
    val terminationRequested: Future[String] = Future.any(List(userInterrupted, Future.delay(10 seconds).continue { case _ => "Timeout" }))
